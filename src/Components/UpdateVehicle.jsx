...
const UpdateVehicle = () => {
  ...
  return (
    <div className="px-6 md:px-10">
      <div className="mx-auto p-8 rounded-2xl shadow my-14">
        <h2 className="text-[2rem] md:text-[2.8rem] font-bold text-center mb-8 mt-2">Update <span className='text-gradient'>Vehicle</span></h2>
        <form onSubmit={handleUpdate} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            <div>
              <label className=" label font-medium mb-1">Vehicle Name</label>
              <input type="text" name="vehicleName" defaultValue={vehicle.vehicleName} className="input w-full" required />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" name="pricePerDay" defaultValue={vehicle.pricePerDay} className="input    w-full" required />
            </div>
            <div>
              <label className=" label font-medium mb-1">Seating Capacity</label>
              <input type="number" name="seatingCapacity" defaultValue={vehicle.seatingCapacity} className="input   w-full" required />
            </div>
            <div>
              <label className=" label font-medium mb-1">Location (City, Country)</label>
              <input type="text" name="location" defaultValue={vehicle.location} className="input w-full" required />
            </div>
            <div>
              <label className=" label font-medium mb-1">Features (comma separated)</label>
              <input type="text" name="features" defaultValue={vehicle.features.join(", ")} className="input w-full" required />
            </div>
            <div className="md:col-span-2">
              <label className="label font-semibold mb-1">Cover Image URL</label>
              <input type="text" name="coverImage" defaultValue={vehicle.coverImage} className="input  w-full" required />
            </div>
            <div className="md:col-span-2">
              <label className="label font-semibold mb-1">Description</label>
              <textarea name="description" defaultValue={vehicle.description} className="textarea textarea-bordered w-full h-24" rows={3} required></textarea>
            </div>
          </div>
