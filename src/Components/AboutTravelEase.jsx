...
const AboutTravelEase = () => {
    ...
    return (
        <div className="px-6 md:px-10">
            ...
            <p className="w-[85%] mx-auto text-center text-[1rem] md:text-[1.1rem] leading-relaxed mt-4 mb-10"> TravelEase is a modern vehicle rental platform built to make travel simple, flexible, and affordable. Whether you’re exploring new cities, heading out for a weekend getaway, or need a reliable ride for everyday tasks — TravelEase connects you with verified vehicles and trusted owners in just a few clicks. </p>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 pb-12">
                {infoCards.map((card) => (
                <div key={card.id} className="rounded-xl text-center p-6 shadow-md border border-gray-100 transform transition duration-300 hover:shadow-xl hover:-translate-y-2">
                    <h3 className="text-[1.3rem] font-bold text-center mb-3">{card.title} </h3>
                    <p className="text-[0.9rem] leading-relaxed"> {card.description} </p>
                </div>))
                }
            </div>
        </div>
